-- VERSO Holdings Portal - Complete Test Data Creation with Auth Users
-- Run this in your Supabase SQL Editor (requires service_role permissions)

BEGIN;

-- 1. CREATE AUTH USERS FIRST (this requires service_role privileges)
-- These INSERT statements should be run by an admin user or service_role
...
INSERT INTO auth.users (
    ...
    ('inv-001', 'investor@demo.com', now(), now(), now(), '{}', '{"display_name": "John Investor"}', false, 'authenticated', crypt('demo123', gen_salt('bf'))),
    ('inv-002', 'sarah@investor.com', now(), now(), now(), '{}', '{"display_name": "Sarah Wilson"}', false, 'authenticated', crypt('demo123', gen_salt('bf'))),
    ('inv-003', 'family.office@demo.com', now(), now(), now(), '{}', '{"display_name": "Wellington Family Office"}', false, 'authenticated', crypt('demo123', gen_salt('bf'))),
    ('staff-001', 'admin@demo.com', now(), now(), now(), '{}', '{"display_name": "Admin User"}', false, 'authenticated', crypt('admin123', gen_salt('bf'))),
    ('staff-002', 'manager@demo.com', now(), now(), now(), '{}', '{"display_name": "Portfolio Manager"}', false, 'authenticated', crypt('manager123', gen_salt('bf'))),
    ('staff-003', 'operations@demo.com', now(), now(), now(), '{}', '{"display_name": "Operations Team"}', false, 'authenticated', crypt('ops123', gen_salt('bf')))
ON CONFLICT (id) DO NOTHING;

-- 2. NOW CREATE PROFILES (referencing the auth.users we just created)
...
SELECT 'Auth users created:' as info, COUNT(*) as count FROM auth.users WHERE id IN ('inv-001', 'inv-002', 'inv-003', 'staff-001', 'staff-002', 'staff-003');
