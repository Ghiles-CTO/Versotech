...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: cpUser, error: cpError } = await serviceSupabase
      ...
      .select('commercial_partner_id, can_sign')
      .eq('user_id', user.id)
      .maybeSingle()
    ...
    const fileName = `commercial-partner-signatures/${cpUser.commercial_partner_id}/${user.id}_${Date.now()}.${fileExt}`
    ...
    if (uploadError) {
      console.error('[cp-upload-signature] Storage error:', uploadError)
      return NextResponse.json({
    ...
    // Update commercial_partner_users record with signature URL
    ...
    if (updateError) {
      console.error('[cp-upload-signature] Update error:', updateError)
      return NextResponse.json({
    ...
    console.error('[cp-upload-signature] Error:', error)
    return NextResponse.json({
