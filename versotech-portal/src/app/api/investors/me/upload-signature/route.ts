...
export async function POST(request: NextRequest) {
  try {
    ...
    const fileName = `investor-signatures/${investorUser.investor_id}/${user.id}_${Date.now()}.${fileExt}`
    ...
    if (uploadError) {
      console.error('[investor-upload-signature] Storage error:', uploadError)
      return NextResponse.json({
    ...
    // Update investor_users record with signature URL
    ...
    if (updateError) {
      console.error('[investor-upload-signature] Update error:', updateError)
      return NextResponse.json({
    ...
    console.error('[investor-upload-signature] Error:', error)
    return NextResponse.json({
...
export async function DELETE() {
  try {
    ...
    if (updateError) {
      console.error('[investor-delete-signature] Update error:', updateError)
      return NextResponse.json({
    ...
    console.error('[investor-delete-signature] Error:', error)
    return NextResponse.json({
      error: 'Internal server error'
    }, { status: 500 })
  }
