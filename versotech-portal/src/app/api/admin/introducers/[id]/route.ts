...
const updateIntroducerSchema = z.object({
  legal_name: z.string().min(2).optional(),
  display_name: z.string().nullable().optional(),
  type: z.enum(['entity', 'individual', 'sole_proprietor']).optional(),
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    // Check introducer exists
    const { data: existingIntroducer, error: fetchError } = await supabase
      .from('introducers')
      .select('id, kyc_status')
      .eq('id', id)
      .single()

    if (fetchError || !existingIntroducer) {
      return NextResponse.json({ error: 'Introducer not found' }, { status: 404 })
    }
    ...
    // Update introducer
    const { data: introducer, error: updateError } = await supabase
      .from('introducers')
      .update({
        ...cleanedData,
    ...
    if (updateError) {
      console.error('Introducer update error:', updateError)
    ...
    console.error('Introducer update error:', error)
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!permissions || permissions.length === 0) {
      return NextResponse.json({ error: 'Only super admins can delete introducers' }, { status: 403 })
    }
    ...
    if (updateError) {
      console.error('Introducer delete error:', updateError)
    ...
    await supabase.from('audit_logs').insert({
      event_type: 'entity_management',
      actor_id: user.id,
      action: 'introducer_deleted',
      entity_type: 'introducer',
      entity_id: id,
      action_details: {
        legal_name: existingIntroducer.legal_name,
      },
      timestamp: new Date().toISOString()
    })
    ...
    console.error('Introducer delete error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
