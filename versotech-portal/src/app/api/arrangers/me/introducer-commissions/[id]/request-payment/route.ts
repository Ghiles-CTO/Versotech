/**
 * Request Payment API
 * POST /api/arrangers/me/introducer-commissions/[id]/request-payment
 *
 * Creates payment request to lawyer/CEO for commission payout.
 * Implements User Story Row 36: Request payment to Lawyer/CEO
 */
...
export async function POST(
  ...
  try {
    ...
    // Fetch commission with introducer and deal info
        ...
        for (const lu of specificLawyerUsers) {
          ...
          if (!notificationsToCreate.some(n => n.user_id === lu.user_id)) {
            notificationsToCreate.push({
              user_id: lu.user_id,
              investor_id: null,
              title: 'Payment Request - Introducer Commission (Priority)',
              message: `Payment of ${formattedAmount} requested for ${introducer?.legal_name || 'Introducer'}${deal ? ` (${deal.name})` : ''}. Please process.`,
              link: `/versotech_main/lawyer-reconciliation`,
              type: 'action_required',
            })
          }
    ...
    if (notificationsToCreate.length > 0) {
      ...
      console.log('[request-payment] Sent', notificationsToCreate.length, 'notifications')
    }
    ...
    await serviceSupabase.from('audit_logs').insert({
      ...
      actor_id: user.id,
      action_details: {
        description: 'Payment requested for introducer commission',
    ...
    console.error('[request-payment] Unexpected error:', error)
