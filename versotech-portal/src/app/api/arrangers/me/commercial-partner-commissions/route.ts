...
export async function POST(request: NextRequest) {
  try {
    ...
    // GAP-9 FIX: Verify commercial partner has a valid signed placement agreement
    // NOTE: Commercial partners use placement_agreements table (not commercial_partner_agreements which doesn't exist)
    const today = new Date().toISOString().split('T')[0]
    const { data: validAgreement } = await serviceSupabase
      .from('placement_agreements')
      .select('id, status, signed_date, expiry_date')
      .eq('commercial_partner_id', data.commercial_partner_id)
      .eq('status', 'active')
    ...
    if (!validAgreement) {
      return NextResponse.json({
        error: 'No valid commercial partner agreement',
    ...
    // Prevent creating multiple commissions for the same (commercial_partner, deal, investor, basis_type)
    if (data.deal_id && data.investor_id) {
      ...
      if (existingCommission) {
        return NextResponse.json({
          ...
          message: 'A commission already exists for this commercial partner, deal, investor, and basis type combination'
        }, { status: 400 })
      }
    }
    ...
    await auditLogger.log({
      ...
      entity: AuditEntities.COMMERCIAL_PARTNER_COMMISSIONS,
