/**
 * Arranger Partner Commission Detail API
 * GET /api/arrangers/me/partner-commissions/[id] - Get commission details
 * PATCH /api/arrangers/me/partner-commissions/[id] - Update commission
 */
...
/**
 * GET /api/arrangers/me/partner-commissions/[id]
 * Get details of a specific partner commission
 */
export async function GET(
  ...
  try {
    ...
    console.error('[arranger/partner-commissions/[id]] Unexpected error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}

/**
 * PATCH /api/arrangers/me/partner-commissions/[id]
 * Update a partner commission (status, notes, etc.)
 */
export async function PATCH(
  ...
  try {
    ...
    if (updateError) {
      console.error('[arranger/partner-commissions/[id]] Error updating:', updateError)
    ...
    await serviceSupabase.from('audit_logs').insert({
      ...
      actor_id: user.id,
      action_details: {
        description: 'Partner commission updated',
        changes: data,
    ...
    const transformed = {
      ...updatedCommission,
      partner: Array.isArray(updatedCommission.partner) ? updatedCommission.partner[0] : updatedCommission.partner,
