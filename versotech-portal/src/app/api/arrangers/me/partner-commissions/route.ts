/**
 * Arranger Partner Commissions API
 * GET /api/arrangers/me/partner-commissions - List commissions for arranger's partners
 * POST /api/arrangers/me/partner-commissions - Create new commission record
 */
...
// Schema for creating a partner commission
...
/**
 * GET /api/arrangers/me/partner-commissions
 * List commissions for partners in arranger's network
 *
 * Query params:
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('[arranger/partner-commissions] Error fetching:', error)
    ...
    console.error('[arranger/partner-commissions] Unexpected error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}

/**
 * POST /api/arrangers/me/partner-commissions
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verify partner exists and is in arranger's network
    ...
    // GAP-9 FIX: Verify partner has a valid signed placement agreement
    // NOTE: Partners use placement_agreements table (not partner_agreements which doesn't exist)
    ...
    if (!validAgreement) {
      return NextResponse.json({
        error: 'No valid partner agreement',
        message: 'Cannot create commission without an active signed placement agreement'
      }, { status: 400 })
    }
    ...
    // Prevent creating multiple commissions for the same (partner, deal, investor, basis_type)
    if (data.deal_id && data.investor_id) {
      ...
      if (existingCommission) {
        return NextResponse.json({
          ...
          message: 'A commission already exists for this partner, deal, investor, and basis type combination'
        }, { status: 400 })
      }
    }
    ...
    if (createError) {
      console.error('[arranger/partner-commissions] Error creating:', createError)
    ...
    await auditLogger.log({
      ...
      entity: AuditEntities.PARTNER_COMMISSIONS,
    ...
    const transformed = {
      ...commission,
      partner: Array.isArray(commission.partner) ? commission.partner[0] : commission.partner,
