...
export async function GET(request: Request, { params }: RouteParams) {
  try {
    ...
    const { data: commercialPartner, error: cpError } = await supabase
    ...
    return NextResponse.json({
      members: members || [],
      commercial_partner: { id: commercialPartner.id, type: commercialPartner.type, name: commercialPartner.name }
    })
  } catch (error) {
