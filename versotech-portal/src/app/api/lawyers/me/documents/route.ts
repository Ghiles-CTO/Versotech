/**
 * Lawyer Documents API (Self-Service)
 * GET /api/lawyers/me/documents - List lawyer's KYC documents
 * POST /api/lawyers/me/documents - Upload a new document
 */
 ...
 * List documents for the current lawyer entity, including member info
 */
export async function GET() {
  try {
    ...
    const { link: lawyerUser, error: lawyerError } = await resolvePrimaryPersonaLink<{
      lawyer_id: string
    }>({
    ...
    if (lawyerError || !lawyerUser?.lawyer_id) {
      return NextResponse.json({ error: 'Lawyer profile not found' }, { status: 404 })
    }

    // Fetch documents for this lawyer entity
    ...
    const formattedDocs = documents?.map(doc => ({
      ...
      lawyer_member_id: doc.lawyer_member_id
    })) || []
    ...
    console.error('Error in GET /api/lawyers/me/documents:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}

/**
 * POST /api/lawyers/me/documents
 * Upload a document for the current lawyer entity
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const lawyerMemberId = formData.get('lawyer_member_id') as string | null
    ...
    let resolvedLawyerMemberId: string | null = null
    if (lawyerMemberId && lawyerMemberId !== 'entity-level') {
      const { data: memberCheck, error: memberError } = await serviceSupabase
        .from('lawyer_members')
        .select('id')
        .eq('id', lawyerMemberId)
      ...
      resolvedLawyerMemberId = memberCheck.id
    }
    ...
    const fileKey = `lawyer-kyc/${lawyerId}/${documentType}/${timestamp}-${randomId}.${fileExt}`
    ...
    const { data: document, error: docError } = await serviceSupabase
      ...
      .insert({
        ...
        lawyer_id: lawyerId,
        lawyer_member_id: resolvedLawyerMemberId,
    ...
    if (resolvedLawyerMemberId) {
      versionQuery = versionQuery.eq('lawyer_member_id', resolvedLawyerMemberId)
    } else {
      versionQuery = versionQuery.is('lawyer_member_id', null)
    }
    ...
    const { data: submission, error: submissionError } = await serviceSupabase
      ...
      .insert({
        ...
        metadata: {
          ...
          source: 'lawyer_profile_upload',
          selected_lawyer_member_id: resolvedLawyerMemberId,
    ...
    await serviceSupabase.from('audit_logs').insert({
      ...
      actor_id: user.id,
      action: 'lawyer_document_uploaded',
    ...
    console.error('Error in POST /api/lawyers/me/documents:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
