 ...
 * POST /api/lawyers/me/partner-commissions/[id]/confirm-payment
 *
 * Marks partner commission as paid and notifies all parties.
 */
  ...
  try {
    ...
    const { data: commission, error: fetchError } = await serviceSupabase
      ...
      .select(`
        ...
        invoice_id,
        notes,
        partner:partner_id(id, name, legal_name, primary_contact_name, primary_contact_email),
      ...
      if (arrangerUsers) {
        for (const au of arrangerUsers) {
          notificationsToCreate.push({
            ...
            message: `Payment of ${formattedAmount} to ${partner?.legal_name || partner?.name || 'Partner'} has been confirmed${deal ? ` for ${deal.name}` : ''}.`,
            link: `/versotech_main/my-partners`,
            type: 'success',
          })
        }
      }
    ...
    if (staffAdmins) {
      for (const admin of staffAdmins) {
        notificationsToCreate.push({
          user_id: admin.id,
          investor_id: null,
          title: 'Partner Payment Confirmed',
          message: `Payment of ${formattedAmount} to ${partner?.legal_name || partner?.name || 'Partner'} has been confirmed by lawyer.`,
    ...
    await serviceSupabase.from('notifications').insert({
      ...
      message: `You confirmed payment of ${formattedAmount} to ${partner?.legal_name || partner?.name || 'Partner'}${deal ? ` for ${deal.name}` : ''}.`,
    ...
    await serviceSupabase.from('audit_logs').insert({
      ...
      actor_id: user.id,
      action_details: {
        description: 'Partner commission payment confirmed by lawyer',
