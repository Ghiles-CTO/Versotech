...
export default async function VehicleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Get current user - unified layout already handles auth checks
  ...
  if (!positionData || positionData.units <= 0) {
    redirect('/versotech_main/portfolio')
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <Card className="border-0 shadow-md">
              ...
              <CardContent>
                ...
                <div className="text-sm text-gray-500 dark:text-gray-400 mt-2">Amount from subscription pack</div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-zinc-700">
                    ...
                    <span className="font-semibold text-gray-900 dark:text-gray-100">{formatPercent(feeStructure.subscriptionFeePercent)}</span>
                  </div>

                  <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-zinc-700">
                    <span className="text-gray-600 dark:text-gray-400">Management Fee</span>
                    <span className="font-semibold text-gray-900 dark:text-gray-100">{formatPercent(feeStructure.managementFeePercent)}</span>
                  </div>

                  <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-zinc-700">
                    <span className="text-gray-600 dark:text-gray-400">Performance Fee (Carry)</span>
                    <span className="font-semibold text-gray-900 dark:text-gray-100">{formatPercent(feeStructure.performanceFeePercent)}</span>
                  </div>

                  {getPerformanceScheduleText() && (
                    <p className="text-xs text-gray-500 dark:text-gray-400">{getPerformanceScheduleText()}</p>
                  )}
                </div>
              </CardContent>
            </Card>
                ...
                {cashflowData && cashflowData.history && cashflowData.history.length > 0 ? (
                  <div className="space-y-3">
                    {cashflowData.history.map((flow, index) => (
                      <div key={index} className="flex items-center justify-between p-4 border dark:border-zinc-700 rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-800">
                        <div className="flex items-center gap-4">
