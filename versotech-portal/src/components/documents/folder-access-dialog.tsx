...
export function FolderAccessDialog({
  ...
  const loadAccessGrants = async () => {
    ...
    try {
      setLoading(true)
      const response = await fetch(`/api/staff/documents/folders/${folderId}/access`)
      if (response.ok) {
  ...
  const handleGrantAccess = async () => {
    if (!selectedInvestorId || !folderId) {
  ...
  const handleRevokeAccess = async (investorId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/staff/documents/folders/${folderId}/access?investor_id=${investorId}`,
        { method: 'DELETE' }
      )
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            Manage Investor Access - {folderName}
          </DialogTitle>
        </DialogHeader>
