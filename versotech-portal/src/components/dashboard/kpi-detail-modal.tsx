    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto">
        <DialogHeader className="pb-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <DialogTitle className="text-2xl font-bold text-foreground">{kpiData.title}</DialogTitle>
        ...
        <div className="mb-10">
          <Card>
            <CardHeader className="pb-8">
              <CardTitle className="text-xl font-semibold text-foreground uppercase tracking-wide">Current Value</CardTitle>
            </CardHeader>
            <CardContent className="space-y-8">
                ...
                {kpiData.trend && (
                  <div className="flex items-center justify-center gap-4 mb-8">
                    <TrendIcon className={cn(
                      ...
                      kpiData.trend.direction === 'down' ? "text-red-600" : "text-muted-foreground"
                    )} />
                    <span className={cn(
              ...
              {/* Benchmark */}
              {kpiData.benchmark && (
                <div className="text-center p-8 bg-muted rounded-xl max-w-md mx-auto">
                  <div className="text-base font-medium text-muted-foreground mb-4 uppercase tracking-wide">
                    {kpiData.benchmark.label}
                  </div>
                  <div className="text-3xl font-bold text-foreground mb-4">{kpiData.benchmark.value}</div>
                  <Badge
                    variant={
        ...
        <div className="mb-10">
          <Card>
            <CardHeader className="pb-8">
              <CardTitle className="text-xl font-semibold text-foreground uppercase tracking-wide">Breakdown</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                {kpiData.breakdown.map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-8 bg-muted rounded-xl border border-border hover:bg-muted/80 transition-colors duration-200">
                    <div className="flex-1">
                      <div className="font-bold text-2xl text-foreground mb-2">{item.label}</div>
                      {item.percentage && (
        ...
        {kpiData.relatedMetrics && kpiData.relatedMetrics.length > 0 && (
          <div className="mb-10">
            <Card>
              <CardHeader className="pb-8">
                <CardTitle className="text-xl font-semibold text-foreground uppercase tracking-wide">Related Metrics</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  {kpiData.relatedMetrics.map((metric, index) => (
                    <div key={index} className="flex items-center justify-between p-8 bg-muted rounded-xl border border-border hover:bg-muted/80 transition-colors duration-200">
                      <div className="flex-1">
                        <div className="font-bold text-2xl text-foreground mb-2">{metric.name}</div>
                      </div>
                      <div className="text-right ml-8">
                        <div className="font-bold text-4xl text-foreground mb-2">{metric.value}</div>
                        <div className={cn(
                          "text-lg font-semibold",
        ...
        {kpiData.insights && kpiData.insights.length > 0 && (
          <div className="mb-10">
            <Card>
              <CardHeader className="pb-8">
                <CardTitle className="text-xl font-semibold text-foreground uppercase tracking-wide flex items-center gap-4">
                ...
                <ul className="space-y-8">
                  {kpiData.insights.map((insight, index) => (
                    <li key={index} className="flex items-start gap-6 text-lg">
                      <div className="h-4 w-4 rounded-full bg-blue-600 dark:bg-blue-500 mt-2 flex-shrink-0" />
