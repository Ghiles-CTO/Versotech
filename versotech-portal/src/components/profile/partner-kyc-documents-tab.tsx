...
interface PartnerKYCDocumentsTabProps {
  partnerId: string
  partnerName?: string
  kycStatus?: string
...
export function PartnerKYCDocumentsTab({
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch('/api/partners/me/documents')
  ...
  const handleUpload = async (file: File, documentType: string, memberId?: string) => {
    ...
    try {
      ...
      if (memberId && memberId !== 'entity-level') {
        formData.append('partner_member_id', memberId)
      }
      ...
      {members.length > 0 && (
        <Card>
          ...
          <CardContent className="space-y-6">
            {members.map((member) => {
              const memberDocs = documents.filter(d => d.partner_member_id === member.id)
