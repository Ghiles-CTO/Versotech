...
const REQUIRED_DOCUMENTS = [
  { label: 'Law Firm Registration Certificate', value: 'law_firm_registration' },
  { label: 'Professional License/Bar Admission', value: 'professional_license' },
  { label: 'Proof of Insurance (Professional Liability)', value: 'professional_liability_insurance' },
  { label: 'Company Registration Document', value: 'company_registration' },
  { label: 'Authorized Signatory List', value: 'authorized_signatories' },
  { label: 'AML/KYC Compliance Certificate', value: 'aml_compliance' },
]
...
export function LawyerKYCDocumentsTab({
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`/api/lawyers/${lawyerId}/documents`)
      ...
      // Don't show error toast - lawyer may not have documents endpoint yet
    } finally {
      setLoading(false)
    }
  }, [lawyerId])
