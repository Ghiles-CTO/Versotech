...
interface CommercialPartnerMember {
  id: string
  full_name: string
  role: string
...
interface CommercialPartnerKYCDocumentsTabProps {
  ...
  const handleUpload = async (file: File, documentType: string, memberId?: string) => {
    ...
    try {
      ...
      if (memberId && memberId !== 'entity-level') {
        formData.append('commercial_partner_member_id', memberId)
      }
      ...
      {members.length > 0 && (
        <Card>
          ...
          <CardContent className="space-y-6">
            {members.map((member) => {
              const memberDocs = documents.filter(d => d.commercial_partner_member_id === member.id)
