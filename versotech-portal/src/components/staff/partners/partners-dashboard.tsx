...
export type PartnersDashboardProps = {
  summary: {
...
export function PartnersDashboard({ summary, partners }: PartnersDashboardProps) {
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage co-investors, syndicate partners, and family offices
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="pb-4">
          ...
          <CardDescription>Browse and manage investment partners</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {filteredPartners.map((partner) => (
                ...
                >
                  <div className="flex items-center justify-between p-4 rounded-lg border border-white/10 hover:bg-white/5 transition-colors">
                    ...
                    <div className="flex items-center gap-4">
                      {partner.typicalInvestmentMin && partner.typicalInvestmentMax && (
                        <div className="hidden md:block text-right">
                          <div className="text-xs text-muted-foreground">Typical Investment</div>
                          <div className="text-sm text-foreground flex items-center gap-1">
                            <DollarSign className="h-3 w-3" />
                            {formatCurrency(partner.typicalInvestmentMin)} - {formatCurrency(partner.typicalInvestmentMax)}
                          </div>
                        </div>
                      )}
                      {partner.preferredSectors && partner.preferredSectors.length > 0 && (
                        <div className="hidden lg:flex gap-1">
                          {partner.preferredSectors.slice(0, 2).map((sector) => (
                            <Badge key={sector} variant="outline" className="text-xs">
                              {sector}
                            </Badge>
                          ))}
                          {partner.preferredSectors.length > 2 && (
      ...
      {/* Add Dialog */}
      <AddPartnerDialog open={addDialogOpen} onOpenChange={setAddDialogOpen} />
    </div>
  )
}
