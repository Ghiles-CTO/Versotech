...
export function buildSubscriptionPackPayload(
  ...
  const wireBankName = normalizeWhitespace(String(feeStructure.wire_bank_name || 'Banque de Luxembourg'))
  const wireBankAddress = normalizeWhitespace(String(feeStructure.wire_bank_address || '14, boulevard Royal, L-2449 Luxembourg, Grand Duchy of Luxembourg'))
  const wireAccountHolder = normalizeWhitespace(String(feeStructure.wire_account_holder || 'Elvinger Hoss Prussen - Escrow Account'))
  const wireEscrowAgent = normalizeWhitespace(String(feeStructure.wire_escrow_agent || 'Elvinger Hoss Prussen'))
  const wireLawFirmAddress = normalizeWhitespace(String(feeStructure.wire_law_firm_address || '2 Place Winston Churchill, L-1340 Luxembourg, Grand Duchy of Luxembourg'))
  const wireIban = normalizeWhitespace(String(feeStructure.wire_iban || 'LU28 0019 4855 4447 1000'))
  const wireBic = normalizeWhitespace(String(feeStructure.wire_bic || 'BLUXLULL'))
