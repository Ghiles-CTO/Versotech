...
async function VersoSignPage() {
    ...
    const isCommercialPartner = personas?.some((p)=>p.persona_type === 'commercial_partner') || false;
    ...
    let commercialPartnerIds = [];
    if (isCommercialPartner) {
        const { data: cpLinks } = await serviceSupabase.from('commercial_partner_users').select('commercial_partner_id').eq('user_id', user.id);
    ...
    if (shouldShowIntroducerTasks && introducerIds.length > 0) {
        const { data: introducerTasks } = await serviceSupabase.from('tasks').select('*').in('kind', [
            ...
            'subscription_pack_signature',
            'other'
        ]).or(`owner_user_id.eq.${user.id},owner_introducer_id.in.(${introducerIds.join(',')})`).order('due_at', {
            ascending: true,
            nullsFirst: false
        });
    ...
    if (shouldShowCPTasks && commercialPartnerIds.length > 0) {
        const { data: cpTasks } = await serviceSupabase.from('tasks').select('*').in('kind', [
            'countersignature',
            'subscription_pack_signature',
            'other'
        ]).or(`owner_user_id.eq.${user.id},owner_commercial_partner_id.in.(${commercialPartnerIds.join(',')})`).order('due_at', {
            ascending: true,
            nullsFirst: false
        });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b32ca6d6._.js.map