...
"[project]/versotech-portal/src/app/api/vehicles/dropdown-options/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        const [{ data: arrangers, error: arrangersError }, { data: lawyers, error: lawyersError }] = await Promise.all([
            // Arrangers from arranger_entities
            serviceSupabase.from('arranger_entities').select('id, legal_name, email, status').or('status.eq.active,status.is.null').order('legal_name', {
                ascending: true
            }),
            // Lawyers from lawyers table
            serviceSupabase.from('lawyers').select('id, firm_name, display_name, primary_contact_email, is_active').or('is_active.eq.true,is_active.is.null').order('firm_name', {
                ascending: true
            })
        ]);
        if (arrangersError) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__cf1cb808._.js.map