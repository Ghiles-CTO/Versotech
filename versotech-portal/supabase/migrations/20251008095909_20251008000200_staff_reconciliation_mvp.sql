-- Bank Reconciliation MVP migration (retry)
BEGIN;
...
WITH numbered AS (
  SELECT id,
         'INV-' || to_char(created_at, 'YYYYMMDD') || '-' || lpad(row_number() OVER (PARTITION BY to_char(created_at, 'YYYYMMDD') ORDER BY id)::text, 4, '0') AS generated_number
