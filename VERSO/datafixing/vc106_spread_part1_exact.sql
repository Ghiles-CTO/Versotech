...
inv_exact as (
    select id as investor_id, legal_name
    from investors
),
inv_norm as (
...
inputs as (
    select d.accrual_amount, d.spread_pps, coalesce(exact.investor_id, norm.investor_id) as investor_id
    from dash d
    left join inv_exact exact
      on lower(exact.legal_name) = lower(d.investor_name)
    left join inv_norm norm
      on norm.norm_name = regexp_replace(lower(d.investor_name), '[^a-z0-9]+', '', 'g')
     and exact.investor_id is null
    where coalesce(exact.investor_id, norm.investor_id) is not null
),
sub as (
