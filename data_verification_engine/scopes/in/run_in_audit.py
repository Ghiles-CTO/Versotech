    ...
    def run_checks(self):
        ...
        if self.rules["checks"].get("specific_rule_assertions", False):
            ...
            for rule in self.rules.get("specific_commission_expectations", []):
                ...
                expected_rate = int(rule["rate_bps"]) if rule.get("rate_bps") not in (None, "") else None
                ...
                if expected_rate is not None and expected_rate not in got["rates"]:
                    self.add_failure(
...
def parse_args():
    p = argparse.ArgumentParser(description="IN deterministic audit")
